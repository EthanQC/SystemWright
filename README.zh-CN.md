...
![示意图:模糊、凌乱的目标进 —— 今天就能手动跑的周度 Loop 出](docs/demo.png)
...
```text
system-wright/            # skill 本体(把这个目录拷进 host 的 skills 文件夹)
  SKILL.md                # 入口:name + description + 工作流
  openai.yaml             # Codex 展示用元数据(其它 host 忽略)
  references/             # 渐进披露的细节,一层深
scripts/
  install.sh              # 跨平台安装脚本
  validate_skill.py       # 自包含、零依赖的校验器
  test_validate_skill.py  # 校验器单测
  check_packaging.py      # manifest + 落地页校验
tests/
  check_skill_quality.py  # 完整性 lint
test-output/              # 设计走查记录 + 负向触发测试
LICENSE
```
...
这些是**设计走查**——自演的、设计自洽的走查,用来展示 skill 端到端的形态;它们**还不是**在真实生产数据上的回放(真实数据的实测会替换它们)。
...
- `actual-test-saas-support.md`:一个 SaaS 客服工作流变成三分类/回复 Loop,含 MCP=yes 与 maker-checker。
...
python3 scripts/check_packaging.py        # manifest + 落地页校验
...
欢迎 issue 与 PR——先看 [CONTRIBUTING.md](CONTRIBUTING.md) 里要先跑的校验命令,报告安全问题见 [SECURITY.md](SECURITY.md)。SystemWright **刻意不做版本号**:没有 version 字段,发布以 `main` 上的 git commit 为准,而非版本号。
...
- Kimi(月之暗面)Anthropic 兼容 API: https://platform.moonshot.ai
- GLM(智谱)编程 API: https://docs.z.ai
